...
def select_actions(q_net, obs_tuple, epsilon):
    ...
    for obs in obs_tuple:
        if random.random() < epsilon:
            ...
            with torch.no_grad():
                obs_t = torch.FloatTensor(flatten_and_pad(obs)).unsqueeze(0)
...
def main():
    ...
    env.unwrapped.config.update({
        "use_pidm": use_pidm,
        "cooperation_level": "mixed",      # 让 agent 见到合作和不合作的 HDV
    })
    ...
    for stage in CURRICULUM:
        ...
        for ep_in_stage in range(n_episodes):
            ...
            for step in range(max_steps):
                ...
                local_rewards = compute_local_rewards(env, next_obs, ramp_av_indices, merged_avs)
